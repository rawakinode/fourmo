...
// Returns: { name, shortName, desc, label, imagePrompt, tagline }
// ---------------------------------------------------------------------------
...
- imagePrompt: vivid detailed prompt for FLUX image generation — character, style (bold flat design, crypto meme aesthetic, vibrant colors, white or solid background), composition. Be specific.
...
app.post('/api/generate-image', async (req, res) => {
  const { prompt, name, shortName } = req.body
  if (!prompt) return res.status(400).json({ error: 'prompt is required' })
  try {
    const enhancedPrompt =
      `Crypto meme coin token logo for "${name}" ($${shortName}). ${prompt}. ` +
      `Style: bold flat design, vibrant saturated colors, clean white circular coin background, ` +
      `iconic mascot character, thick outlines, professional crypto logo, centered composition, no text overlays, square format.`
    ...
    if (AI_IMAGE_PROVIDER === 'dgrid') {
      ...
      const response = await fireworksImgApi.post(`/workflows/${IMAGE_MODEL}/text_to_image`, {
        prompt: enhancedPrompt, aspect_ratio: '1:1', guidance_scale: 4.5, num_inference_steps: 30,
      })
